...
void addEmployee() {
	...
	employees& new_emp = employee[index];

	while (true) {
		cout << "Enter 5-digit ID: ";
		cin >> temp_input;
		if (regex_match(temp_input, regex("^[1-9][0-9]{4}$"))) {
			new_emp.emp_id = temp_input;
			break;
		}
	...
	while (true) {
		...
		if (regex_match(temp_input, regex("^[a-zA-Z\\s]+$"))) {
			new_emp.emp_name = temp_input;
			break;
		}
		cout << "[Error] Letters and spaces only!\n";
	}

	while (true) {
		cout << "Enter Age (18-99): ";
		cin >> temp_input;
		if (regex_match(temp_input, regex("^[1-9][0-9]$"))) {
			new_emp.emp_age = stoi(temp_input);
			break;
		}
	...
	cin.ignore(numeric_limits<streamsize>::max(), '\n');
	cout << "Enter Position: ";
	getline(cin, new_emp.emp_Position);

	while (true) {
		cout << "Enter Basic Salary: ";
		cin >> temp_input;
		if (regex_match(temp_input, regex("^[0-9]+(\\.[0-9]+)?$"))) {
			new_emp.BasicSalary = stod(temp_input);
			break;
		}
		cout << "[Error] Invalid Salary!\n";
	}

	while (true) {
		cout << "Enter Phone (11 digits): ";
		cin >> temp_input;
		if (regex_match(temp_input, regex("^01[0-9]{9}$"))) {
			new_emp.emp_phone = temp_input;
			break;
		}
	...
	while (true) {
		...
		cin >> new_emp.emp_pass;
		if (new_emp.emp_pass.length() >= 6) break;
		cout << "[Error] Password too short!\n";
	}

	cout << "\n[Success] Employee " << new_emp.emp_name << " added!\n";
